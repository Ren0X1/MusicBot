    ...
    try {
        if (!interaction?.guild) {
            return interaction?.reply({
                content: "Este bot es solo para servidores y se puede usar en servidores.",
                ephemeral: true
            })
        } else {
                                ...
                                try {
                                    ...
                                    if (data?.channels?.length > 0) {
                                        ...
                                        if (!channel_filter?.length > 0 && !interaction?.member?.permission?.has("0x0000000000000020")) {
                                            ...
                                            return interaction?.reply({
                                                content: lang.msg126.replace("{channel_filter}", channel_filter),
                                                ephemeral: true
                                            }).catch(e => {})
                                        }
                                    }
                                    if (interaction?.member?.permissions?.has(props?.permissions || "0x0000000000000800")) {
                                        ...
                                        if (props && DJ.commands.includes(interaction?.commandName)) {
                                            let djRole = await db?.musicbot?.findOne({
                                                guildID: interaction?.guild?.id
                                            }).catch(e => {});
                                            if (djRole) {
                                        ...
                                        return interaction?.reply({
                                            content: `${lang.message3}: **${props?.permissions?.replace("0x0000000000000020", "MANAGE GUILD")?.replace("0x0000000000000800", "SEND MESSAGES") || "SEND MESSAGES"}**`,
                                            ephemeral: true
                                        });
                                    }
                                } catch (e) {
