...
module.exports = {
  name: "channel",
  description: "Le permite establecer el canal o los canales donde se puede usar el bot.",
  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      if (stp === "add") {
        ...
        if (channel_filter?.length > 0) return interaction.reply({
          content: lang.msg124,
          ephemeral: true
        }).catch(e => {})

        await db.musicbot.updateOne({
          guildID: interaction.guild.id
        }, {
          $push: {
            channels: {
