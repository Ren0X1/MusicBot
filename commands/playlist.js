...
module.exports = {
    name: "playlist",
    description: "Te permite administrar los comandos de la lista de reproducción.",
    options: [{
            name: "create",
            ...
            options: [{
                    name: "name",
                    description: "Escribe el nombre de la lista de reproducción que quieres crear.",
                    type: ApplicationCommandOptionType.String,
                    required: true
                },
                {
                    name: "public",
                    description: "Hacer la lista de reproducción pública. (true=public playlist, false=private playlist)",
        ...
        },
        {
            ...
            options: [{
                name: "name",
                description: "Escribe el nombre de la lista de reproducción que quieres eliminar.",
                type: ApplicationCommandOptionType.String,
                required: true
            }]
        },
        {
            ...
            options: [{
                    name: "playlist-name",
                    description: "Escribe una lista de reproducción.",
                    type: ApplicationCommandOptionType.String,
                    required: true
                },
                {
        ...
        {
            name: "delete-music",
            description: "Te permite eliminar la música a la lista de reproducción.",
            type: ApplicationCommandOptionType.Subcommand,
            options: [{
                    name: "playlist-name",
                    description: "Escribe una lista de reproducción.",
                ...
                {
                    name: "name",
                    description: "Escribe un nombre de música.",
                    type: ApplicationCommandOptionType.String,
                    required: true
                }
            ]
        ...
        {
            name: "list",
            description: "Explorar música en una lista de reproducción.",
            type: ApplicationCommandOptionType.Subcommand,
            options: [{
                name: "name",
                description: "Escribe una lista de reproducción.",
        ...
        {
            name: "lists",
            description: "Explore todas tus listas de reproducción.",
            type: ApplicationCommandOptionType.Subcommand,
            options: []
        },
        {
    ...
    run: async (client, interaction) => {
        ...
        try {
            ...
            if (stp === "create") {
                ...
                if (userplaylist?.playlist?.length >= client.config.playlistSettings.maxPlaylist) return interaction.reply({
                    content: lang.msg93,
                    ephemeral: true
                }).catch(e => {})
            ...
            if (stp === "delete") {
                ...
                if (!playlist?.playlist?.filter(p => p.name === name).length > 0) return interaction.reply({
                    content: lang.msg96,
                    ephemeral: true
                }).catch(e => {})
            ...
            if (stp === "add-music") {
                ...
                if (!playlist_name) return interaction.reply({
                    content: lang.msg100,
                    ephemeral: true
                }).catch(e => {})
                ...
                if (!playlist?.playlist?.filter(p => p.name === playlist_name).length > 0) return interaction.reply({
                    content: lang.msg96,
                    ephemeral: true
                }).catch(e => {})

                let max_music = client.config.playlistSettings.maxMusic
                if (playlist?.musics?.filter(m => m.playlist_name === playlist_name).length > max_music) return interaction.reply({
                ...
                if (music_filter?.length > 0) return interaction.editReply({
                    content: lang.msg104,
                    ephemeral: true
                }).catch(e => {})
            ...
            if (stp === "delete-music") {
                ...
                if (!music_filter?.length > 0) return interaction.reply({
                    content: lang.msg54,
                    ephemeral: true
                }).catch(e => {})
            ...
            if (stp === "list") {
                ...
                let cmds = await db.playlist_timer.findOne({
                    userID: interaction.user.id,
                    guildID: interaction.guild.id,
                ...
                const playlist = await db.playlist.find().catch(e => {})
                if (!playlist?.length > 0) return interaction.reply({
                    content: lang.msg96,
                    ephemeral: true
                }).catch(e => {})
                ...
                for (let i = 0; i < playlist.length; i++) {
                    if (playlist[i]?.playlist?.filter(p => p.name === name)?.length > 0) {
                        ...
                        if (!trackl?.length > 0) return interaction.reply({
                            content: lang.msg111,
                            ephemeral: true
                        }).catch(e => {})
                ...
                const generateEmbed = async (start) => {
                    ...
                    if (!current || !current?.length > 0) return interaction.reply({
                        content: lang.msg111,
                        ephemeral: true
                    }).catch(e => {})
...
`\n\`${sayı++}\` | [${data.music_name}](${data.music_url}) - <t:${Math.floor(data.saveTime / 1000)}:R>`
                ...
                await interaction.reply({
                    ...
                    await db.playlist_timer.updateOne({
                        userID: interaction.user.id,
                        guildID: interaction.guild.id,
                        channelID: interaction.channel.id
                    }, {
            ...
            if (stp === "lists") {
                ...
                if (!playlist?.playlist?.length > 0) return interaction.reply({
                    content: lang.msg117,
                    ephemeral: true
                }).catch(e => {})
...
`\n**${number++} |** \`${data.name}\` - **${playlist?.musics?.filter(m => m.playlist_name === data.name)?.length || 0}** ${lang.msg116} (<t:${Math.floor(data.createdTime / 1000)}:R>)`
            ...
            if (stp === "top") {
                ...
                if (!playlists?.length > 0) return interaction.reply({
                    content: lang.msg114,
                    ephemeral: true
                }).catch(e => {})

                let cmds = await db.playlist_timer2.findOne({
                    userID: interaction.user.id,
                    guildID: interaction.guild.id,
                    channelID: interaction.channel.id
                }).catch(e => {});
...
`\n**${sayı++} |** \`${data.name}\` By. \`${data.authorTag}\` - **${data.plays}** ${lang.msg129} (<t:${Math.floor(data.createdTime / 1000)}:R>)`
                ...
                await interaction.reply({
                    ...
                    await db.playlist_timer2.updateOne({
                        userID: interaction.user.id,
                        guildID: interaction.guild.id,
                        channelID: interaction.channel.id
                    }, {
